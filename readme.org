...
** Emacs server (local + daemon)

Start a local Unix-socket server in interactive sessions so ~emacsclient~ (and
agents) can open files / eval Lisp in the running Emacs. The macOS daemon path
still uses TCP with ~EMACS_SERVER_AUTH_KEY~ via ~launchctl~.

#+begin_src emacs-lisp
(defun shanemcd/start-emacs-server ()
  "Start Emacs server: TCP when daemonized, local socket otherwise."
  (require 'server)
  (unless (server-running-p)
    (if (daemonp)
        (progn
          (setq server-use-tcp t
                server-auth-key (getenv "EMACS_SERVER_AUTH_KEY")
                server-port 52698)
          (server-start)
          ;; Adjust "mac" if needed for your display configuration.
          (make-frame '((display . "mac"))))
      ;; Interactive / Linux GUI: default local socket under
      ;; /run/user/$UID/emacs/server (or /tmp/emacs$UID/server).
      (setq server-use-tcp nil)
      (server-start))))

(add-hook 'after-init-hook #'shanemcd/start-emacs-server)

;; For agents / emacsclient --eval: open FILE in another window and leave
;; the selected window (e.g. agent-shell) alone.
(defun shanemcd/display-file-other-window (file)
  "Visit FILE in another window without selecting it."
...
Uses [[https://github.com/chrishrb/go-grip][go-grip]] for local offline GFM preview (no GitHub API rate limits). Toggle
with ~C-c C-c g~ in Markdown buffers; preview opens in the browser and refreshes
on save.
...
(use-package grip-mode
  ...
  :bind (:map markdown-mode-command-map
         ("g" . grip-mode))
  :config
  ;; Ensure ~/.local/bin (go-grip) is visible even if Emacs was started
  ;; before the binary was installed.
...
(add-hook 'yaml-mode-hook #'my/yaml-outline-setup)
#+END_SRC
...
Terminal emulator from NonGNU/MELPA. The package ships Linux-compiled
terminfo databases; on macOS/BSD those make Backspace insert spaces
(and other keys misbehave). Recompile once per OS with
~eat-compile-terminfo~; Linux can keep the prebuilt files.

#+begin_src emacs-lisp
(use-package eat
  :commands (eat eat-other-window)
  :config
  ;; MELPA terminfo is Linux-built; Darwin/BSD need a local recompile.
  ;; Stamp is per-system-type so a shared emacs.d across machines is fine.
  (when (memq system-type '(darwin berkeley-unix))
    (let ((stamp (expand-file-name
                  (format ".terminfo-compiled-%s" system-type)
                  eat-term-terminfo-directory)))
      (unless (file-exists-p stamp)
        (eat-compile-terminfo)
        (with-temp-file stamp (insert "ok\n"))))))
#+end_src
...
(use-package auto-dark
  :custom
  (auto-dark-themes '((base16-tomorrow-night) (base16-one-light)))
...
(use-package gptel
  ...
  ;; Define a custom Vertex AI backend that doesn't send model in request body
  (cl-defstruct (gptel-vertex-anthropic (:constructor gptel--make-vertex-anthropic)
                                        (:copier nil)
                                        (:include gptel-anthropic)))

  ;; Override request building to exclude :model field
  (cl-defmethod gptel--request-data ((backend gptel-vertex-anthropic) prompts)
    (let ((data (cl-call-next-method)))
      ;; Remove :model from the plist since Vertex has it in the URL
      (cl-remf data :model)
      data))

  ;; Map Claude Code model aliases to Vertex AI model names
  (let* ((model-alias (or (getenv "ANTHROPIC_MODEL") "claude-sonnet-4-5@20250929"))
         (model-map '(("opusplan" . "claude-sonnet-4-5@20250929")
                      ("opus" . "claude-opus-4-6@20250514")
                      ("sonnet" . "claude-sonnet-4-5@20250929")
                      ("haiku" . "claude-3-5-haiku@20241022")))
         (model (or (cdr (assoc model-alias model-map)) model-alias))
         (region (or (getenv "CLOUD_ML_REGION") "us-east5"))
         (project (getenv "ANTHROPIC_VERTEX_PROJECT_ID")))
    (setq-default gptel-backend
                  (gptel--make-vertex-anthropic
                   :curl-args nil
                   :name "Claude via Vertex"
                   :host (format "%s-aiplatform.googleapis.com" region)
                   :header (lambda (_)
                             `(("Authorization" . ,(concat "Bearer " (gptel--get-api-key)))))
                   :key (lambda ()
                          (with-temp-buffer
                            (call-process "gcloud" nil t nil "auth" "print-access-token")
                   ...
                   :url (format "https://%s-aiplatform.googleapis.com/v1/projects/%s/locations/%s/publishers/anthropic/models/%s:streamRawPredict"
                                region project region model)))
    (setq-default gptel-model (intern model))))
#+end_src
**** ~claude~
#+begin_src emacs-lisp
...
**** ~gptel~ (Cursor models via open-cursor proxy)

Talks to a local OpenAI-compatible proxy on ~127.0.0.1:32124~ that shells out to
the Cursor ~agent~ CLI (same approach as
[[https://github.com/Nomadcxx/opencode-cursor][Nomadcxx/opencode-cursor]]).

Prerequisites:
- ~agent login~ (Cursor Agent CLI)
- ~systemctl --user enable --now open-cursor-proxy~ (serves ~/v1/chat/completions~)
- Optional: ~CURSOR_ACP_SESSION_RESUME=1~ for incremental multi-turn via ~--resume~
...
(use-package gptel
  ...
  :config
  (setq gptel-model 'composer-2.5-fast
        gptel-backend
        (gptel-make-openai "Cursor"
...
**** ~agent-shell~ (Cursor + Claude/Vertex + Gemini via ACP)

Native Emacs shell for LLM agents over [[https://agentclientprotocol.com][ACP]].
Loaded from a local checkout for development
([[https://github.com/xenodium/agent-shell][xenodium/agent-shell]]); ~acp.el~ and ~shell-maker~ still come from
MELPA.

Providers:
- *Cursor* — ~agent acp --approve-mcps~ (existing ~agent login~)
- *Claude* — ~claude-agent-acp~ over Vertex AI (~CLAUDE_CODE_USE_VERTEX~ +
  ~gcloud~ ADC; project/region from ~/.zshrc via chezmoi)
- *Gemini* — ~gemini --acp~ over Vertex AI (~GOOGLE_CLOUD_*~ /
  ~GEMINI_MODEL~ from ~/.zshrc via chezmoi). Antigravity (~agy~) has no
  ACP yet, so we keep Gemini CLI for agent-shell.

Prerequisites on ~PATH~ (via ~exec-path-from-shell~): ~agent~,
~claude-agent-acp~ (~npm i -g @agentclientprotocol/claude-agent-acp~),
~gemini~ (~npm i -g @google/gemini-cli~).

Past conversations live as Markdown under ~.agent-shell/transcripts/~.
[[https://github.com/Marx-A00/agent-recall][agent-recall]] indexes them so you can search, browse, and resume — no
~desktop-save-mode~ / agent-shell-desktop required. Prefer
~session/load~ so resumed buffers get prior messages back.

#+begin_src emacs-lisp
;; Dependencies still from MELPA (not vendored in the local checkout).
(use-package acp :ensure t)
(use-package shell-maker :ensure t)

(use-package agent-shell
  :ensure nil
  :load-path "~/github/xenodium/agent-shell"
  :demand t
  :bind (("C-c A" . agent-shell-cursor-start-agent)
         ("C-c C" . agent-shell-anthropic-start-claude-code)
         ("C-c G" . agent-shell-google-start-gemini))
  :config
  ;; Cursor — `agent login` outside Emacs; no ACP authenticate request.
  (setq agent-shell-cursor-authentication
        (agent-shell-cursor-make-authentication :none t)
        agent-shell-cursor-acp-command '("agent" "acp" "--approve-mcps"))

  ;; Claude — Vertex via env from ~/.zshrc (CLAUDE_CODE_USE_VERTEX, etc.).
  (setq agent-shell-anthropic-authentication
        (agent-shell-anthropic-make-authentication :login t))

  ;; Gemini — Vertex via env from ~/.zshrc (chezmoi); model via GEMINI_MODEL.
  (setq agent-shell-google-authentication
        (agent-shell-google-make-authentication :vertex-ai t)
        agent-shell-google-gemini-acp-command '("gemini" "--acp"))

  ;; Environments are set after exec-path-from-shell (later in this file).

  (setq agent-shell-preferred-agent-config '(preselect . cursor)
        agent-shell-session-strategy 'prompt
        ;; Use session/load (not session/resume) so history is restored.
        agent-shell-prefer-session-resume nil)

  ;; Typing anywhere in the transcript jumps to the prompt (like chat UIs).
  ;; Note: n/p/r stay navigation/quote when not at the prompt.
  (defun shanemcd/agent-shell-self-insert (&optional n)
    "Insert N characters at the prompt, moving point there if needed."
    (interactive "p")
  ...
  (define-key agent-shell-mode-map [remap self-insert-command]
              #'shanemcd/agent-shell-self-insert))

;; Search / browse / resume agent-shell transcripts.
;; Local checkout of https://github.com/shanemcd/agent-recall
;; (branch fix/session-id-and-resume-loop) until upstream merges.
;; Keys: C-c B browse, C-c R resume, C-c / live search.
(use-package agent-recall
  :ensure nil
  :load-path "~/github/shanemcd/agent-recall"
  :after agent-shell
  :hook (agent-shell-mode . agent-recall-track-sessions)
  :bind (("C-c B" . agent-recall-browse)
         ("C-c R" . agent-recall-resume)
         ("C-c /" . agent-recall-search-live))
  :config
  (setq agent-recall-search-paths '("~/github" "~/projects" "~/.emacs.d")
        agent-recall-extra-transcript-dirs
        '((:dir "~/.agent-shell/transcripts/"))
        agent-recall-search-function 'consult-ripgrep
        agent-recall-browse-sort 'modified-desc))

;; Persistent project-aware sidebar for agent-shell.
;; https://github.com/cmacrae/agent-shell-sidebar
;; Keys: C-c s toggle, C-c f focus (C-c a is already ai-code-menu).
(use-package agent-shell-sidebar
  :ensure nil
  :quelpa (agent-shell-sidebar :fetcher github
                               :repo "cmacrae/agent-shell-sidebar")
  :after agent-shell
  :bind (("C-c s" . agent-shell-sidebar-toggle)
...
**** ~ai-code~ (unified AI coding menu)

[[https://github.com/tninja/ai-code-interface.el][ai-code-interface.el]] — transient menu for context-aware coding
actions. Uses ~agent-shell~ as the backend. Menu is ~C-c a~; start
Cursor/Claude/Gemini shells with ~C-c A~ / ~C-c C~ / ~C-c G~.
...
(use-package flycheck
  :delight
  :config
  ;; Silence the once-per-mode "no syntax checker can run here" notice in
  ;; lisp-interaction-mode buffers. Their only checker (the emacs-lisp byte
  ;; compiler) is disabled there by Emacs 30+ trusted-content rules
  ;; (CVE-2024-53920), so Flycheck has nothing to do in those buffers.
...
#+begin_src emacs-lisp
;; Use Emacs 30's built-in :vc support instead of quelpa
(use-package marp-mode
  :vc (:url "https://github.com/shanemcd/marp-mode"
       :branch "main")
...
(use-package exec-path-from-shell
  ...
  (exec-path-from-shell-copy-envs '("GOPATH"
                                    ...
                                    "GEMINI_SANDBOX"
                                    "SANDBOX_FLAGS"
                                    "CURSOR_ACP_SESSION_RESUME"))
  ;; agent-shell is loaded earlier; snapshot env now that PATH/Vertex vars exist.
...
*** Ensure ~HOME~ is not a symlink
On Fedora Kinoite and other Atomic Desktops, ~/home~ is symlinked to
